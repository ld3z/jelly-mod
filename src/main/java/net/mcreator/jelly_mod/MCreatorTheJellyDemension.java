...
public class MCreatorTheJellyDemension extends Elementsjelly_mod.ModElement {
	@ObjectHolder("jelly_mod:thejellydemension")
	public static final ModDimension dimension = null;
	@ObjectHolder("jelly_mod:thejellydemension")
	public static final Item block = null;
	@ObjectHolder("jelly_mod:thejellydemension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;

	public MCreatorTheJellyDemension(Elementsjelly_mod instance) {
		super(instance, 16);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("thejellydemension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("jelly_mod:thejellydemension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("jelly_mod:thejellydemension"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("jelly_mod:thejellydemension"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("plains")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("desert")), ForgeRegistries.BIOMES.getValue(new ResourceLocation("taiga")),
	...
	public void initElements() {
		...
		elements.items.add(() -> new ModTrigger().setRegistryName("thejellydemension"));
	}

	public static class ModTrigger extends Item {
		public ModTrigger() {
			super(new Item.Properties().group(MCreatorModTab.tab).maxDamage(64));
		}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("thejellydemension_portal");
		}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == MCreatorJellyPortalBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == MCreatorJellyPortalBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == MCreatorJellyPortalBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
				...
				for (int j7 = -1; j7 <= 1; ++j7) {
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos, flag ? MCreatorJellyPortalBlock.block.getDefaultState().getBlock()
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, MCreatorJellyPortalBlock.block.getDefaultState().getBlock()
								.getDefaultState(), 3);
					}
				}
			}
		...
		public BiomeProviderCustom(World world) {
			...
			for (Biome biome : this.biomes) {
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(Blocks.GRASS_BLOCK.getDefaultState().getBlock(), biome.getSurfaceBuilder().getConfig()
