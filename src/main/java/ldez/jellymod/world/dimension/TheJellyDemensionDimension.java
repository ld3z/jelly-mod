...
package ldez.jellymod.world.dimension;
...
import ldez.jellymod.item.TheJellyDemensionItem;
import ldez.jellymod.block.JellyPortalBlockBlock;
...
public class TheJellyDemensionDimension extends JellyModElements.ModElement {
	@ObjectHolder("jellymod:thejellydemension")
	public static final ModDimension dimension = null;
	@ObjectHolder("jellymod:thejellydemension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public TheJellyDemensionDimension(JellyModElements instance) {
		super(instance, 17);
	...
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("jellymod:thejellydemension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("jellymod:thejellydemension"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("jellymod:thejellydemension"));
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new TheJellyDemensionItem().setRegistryName("thejellydemension"));
	}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == JellyPortalBlockBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == JellyPortalBlockBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == JellyPortalBlockBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos,
									flag ? JellyPortalBlockBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, JellyPortalBlockBlock.block.getDefaultState().getBlock().getDefaultState(),
								3);
					}
				}
			}
